...
function createUserFromLDAP(ldapResponse, login) {
    ...
    const name = data[0];
    const isEstudent = data.findIndex(
        element => element.toLowerCase() === 'estudiantes'
        ) !== -1;
...
module.exports = {
    checkUserCredentials: (args.env === 'prod' ? checkUserCredentials : mockFuntions.checkUserCredentials),
    checkIfUserExists: (args.env === 'prod' ? checkIfUserExists : mockFuntions.checkIfUserExists),
    createUserFromLDAP,
};
